...
1. **Contradictions**: Find instructions, rules, or statements within the same section that tell the model to do opposite things. Look especially at:
   ...
   - **WITHIN A SINGLE RULE**: Look for rules where the first part establishes a protection/restriction (e.g., "drop any fix that removes Y", "never remove Y") but then a later sentence in the same rule provides an exception that directly violates that protection (e.g., "Y can always be removed", "Y is optional and can be removed")
